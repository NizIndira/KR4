from api.any_api import AnyAPI
from api.get_data_api import GetData
from src.exceptions import APIDataException, GetDataException
from settings import SJ_API_URL, SJ_API_KEY, RESULTS_PER_PAGE
from src.validate_key import validate_key


class SuperJobAPI(AnyAPI, GetData):
    sj_api_url = SJ_API_URL
    sj_api_key = SJ_API_KEY

    def get_vacancies(self, search_string) -> list[dict] | None:
        """Метод возвращает список вакансий по заданному запросу, используя SuperJob API."""
        vacancies = []
        current_page = 0
        ...
        while True:
            try:  # Получаем данные о вакансиях используя SuperJob API
                data = self.get_data(url=self.sj_api_url,
                                            headers=headers,
                                            params=request_params)
            except GetDataException as err:  # Если произошли ошибки, то возвращаем None
                print(err.message)

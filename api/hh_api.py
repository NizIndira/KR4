...
class HeadHunterAPI(AnyAPI, GetData):

    hh_api_url = HH_API_URL

    def get_vacancies(self, search_string) -> list[dict] | None:
        """Метод возвращает список вакансий по заданному запросу, используя HeadHunter API."""
        vacancies = []
        current_page = 0
        ...
        while True:
            try:  # Получаем данные о вакансиях используя HeadHunter API
                data = self.get_data(url=self.hh_api_url, params=request_params)
